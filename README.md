...
For the `parquet` and `delta` output formats, this attribute specifies the output path where the scored data from the predict step is wrote to. In Databricks, this path will be written to be under [DBFS](https://docs.databricks.com/dbfs/index.html). For the `table` output format, this attribute specifies the table name that is used to create a metastore entry.
<u>Example</u>: 
  ```
...
Specifies the save mode used by Spark for writing data. See the [PySpark save modes documentation](https://spark.apache.org/docs/latest/sql-data-sources-load-save-functions.html#save-modes) for more information. Defaults to `default`.
</details>

**Step artifacts**:
- `scored_data`: the scored dataset, with model predictions as the `prediction` column, as a Pandas DataFrame.
